...
function stableAutoDraftId(row, venue, month, year) {
  ...
  return `auto-${year}-${venue}-${monthLabels.indexOf(month) + 1}-${normalizeCustomerId(row.id) || "noid"}-${draftKeyHash(key)}`;
}

function draftKeyHash(key) {
  let hash = 0;
...
function contractEndDateKeyForRow(row) {
  const end = parseFullRocDateParts(row?.end);
  return end ? formatWesternDate(end.westernYear, end.monthNumber, end.day) : "";
}

function renewalEventKeyForRow(row, venue) {
  const customerId = normalizeCustomerId(row?.id);
  const contractEnd = contractEndDateKeyForRow(row);
  return renewalEventKeyForDate(venue, customerId, contractEnd);
}

function renewalEventKeyForDate(venue, customerId, contractEnd) {
  const normalizedId = normalizeCustomerId(customerId);
  if (!venue || !normalizedId || !contractEnd) return "";
  return ["renewal-event", venue, normalizedId, contractEnd].join("|");
}

function renewalEventDateKeyForManualDraft(item, row) {
  const due = String(item?.due || "").trim();
  const monthDay = due.match(/^(\d{1,2})\s*[\/.-]\s*(\d{1,2})$/);
  const westernYear = Number(item?.year);
  if (monthDay && westernYear > 1911) {
    const monthNumber = Number(monthDay[1]);
    const day = Number(monthDay[2]);
    if (monthNumber >= 1 && monthNumber <= 12 && day >= 1 && day <= 31) {
      return formatWesternDate(
        westernYear,
        monthNumber,
        Math.min(day, daysInMonth(westernYear, monthNumber)),
      );
    }
  }
  const fullDate = parseFullRocDateParts(due);
  if (fullDate) {
    return formatWesternDate(fullDate.westernYear, fullDate.monthNumber, fullDate.day);
  }
  return contractEndDateKeyForRow(row);
}

function stableRenewalLeadDraftId(row, venue, notice) {
  const eventKey = renewalEventKeyForRow(row, venue);
  const key = eventKey || ["renewal-event", venue, normalizeCustomerId(row.id), notice.contractDateKey].join("|");
  return `renewal-${notice.year}-${venue}-${notice.monthNumber}-${normalizeCustomerId(row.id) || "noid"}-${draftKeyHash(key)}`;
}
...
function autoDraftForRow(row, venue, month, year) {
  ...
  const renewalEventKey = isRenewal ? renewalEventKeyForRow(row, venue) : "";
  return {
    ...
    autoGenerated: true,
    ...(renewalEventKey ? { renewalEventKey } : {}),
  };
}
...
function renewalLeadDraftForRow(row, venue, sourceMonth, sourceYear) {
  ...
  const titleName = displayNameFor(row);
  const renewalEventKey = renewalEventKeyForRow(row, venue);
  return {
...
function normalizeManualDraftItem(item) {
  ...
  const normalizedIsRenewal =
    String(normalized.kind || "").includes("續約")
    || (normalized.messages || []).some((message) => isRenewalDraftMessage(normalized, message));
  const currentEndDateKey = contractEndDateKeyForRow(row);
  const manualEventDateKey = normalizedIsRenewal
    ? renewalEventDateKeyForManualDraft(normalized, row)
    : "";
  if (
    normalizedIsRenewal
    && manualEventDateKey
    && currentEndDateKey
    && manualEventDateKey < currentEndDateKey
  ) {
    return null;
  }
  if (
  ...
  const renewalEventKey = normalizedIsRenewal || rowIsRenewal
    ? renewalEventKeyForDate(
        ref.venue || normalized.venue,
        row.id || ref.id,
        manualEventDateKey || currentEndDateKey,
      )
    : "";
  if (renewalEventKey) normalized.renewalEventKey = renewalEventKey;
  if (normalizedIsRenewal) {
    if (rowResolvedForDraft(row)) return null;
    return {
      ...normalized,
      kind: "續約",
      renewalEventKey,
  ...
  if (!rowIsRenewal && (normalizedIsRenewal || normalizedHasBlankBody)) {
    ...
    return paymentDraft
      ? {
          ...
          persistedDraft: normalized.persistedDraft,
          persistedStatus: normalized.persistedStatus,
        }
      : null;
  }
  ...
  return {
    ...
    persistedDraft: normalized.persistedDraft,
    persistedStatus: normalized.persistedStatus,
    renewalEventKey: renewalEventKey || renewalDraft.renewalEventKey,
    messages: normalized.messages?.some((message) => String(message?.body || "").trim())
      ? normalized.messages
      : renewalDraft.messages,
  };
}
...
function dedupeManualDraftItems(items) {
  ...
  items.forEach((item) => {
    ...
    key
      ...
      .filter(Boolean)
      .forEach((singleKey) => {
        const storageKey = `${singleKey}::${item.id || ""}`;
        const existing = seen.get(storageKey);
        if (!existing) {
          seen.set(storageKey, item);
          return;
        }
        const messageKeys = new Set((existing.messages || []).map((message) => `${message.label || ""}\n${message.body || ""}`));
...
function isRenewalDraftItem(item) {
  return Boolean(item?.renewalEventKey || String(item?.kind || "").includes("續約"));
}

function mergeRenewalDraftItems(items) {
  const persistedFollow = items.find((item) =>
    item.persistedDraft && ["posted_waiting", "sent"].includes(String(item.persistedStatus || "")),
  );
  const base = persistedFollow || items.find((item) => item.renewalLead) || items[0];
  const merged = { ...base };
  const messageKeys = new Set();
  merged.messages = [];
  merged.paymentRefs = [];
  const paymentRefKeys = new Set();
  items.forEach((item) => {
    (item.messages || []).forEach((message) => {
      const key = `${message.label || ""}\n${message.body || ""}`;
      if (messageKeys.has(key)) return;
      messageKeys.add(key);
      merged.messages.push(message);
    });
    (item.paymentRefs || []).forEach((ref) => {
      const key = [ref.venue || item.venue, Number(ref.year || item.year), ref.month || item.month, normalizeCustomerId(ref.id)].join("|");
      if (paymentRefKeys.has(key)) return;
      paymentRefKeys.add(key);
      merged.paymentRefs.push(ref);
    });
  });
  const notifiedDates = items.map((item) => item.lastNotifiedAt).filter(Boolean).sort();
  if (notifiedDates.length) merged.lastNotifiedAt = notifiedDates.at(-1);
  if (persistedFollow) merged.status = "follow";
  merged.renewalEventKey = base.renewalEventKey || items.find((item) => item.renewalEventKey)?.renewalEventKey;
  return merged;
}

function collapseRenewalEvents(items) {
  const grouped = new Map();
  const loose = [];
  items.forEach((item) => {
    if (!isRenewalDraftItem(item) || !item.renewalEventKey) {
      loose.push(item);
      return;
    }
    const group = grouped.get(item.renewalEventKey) || [];
    group.push(item);
    grouped.set(item.renewalEventKey, group);
  });
  return [...loose, ...Array.from(grouped.values(), mergeRenewalDraftItems)];
}
