...
function samePaymentPeriod(a, b) {
  return ["id", "company", "cycle", "start", "end"].every((field) => {
    const left = field === "start" || field === "end" ? comparableDate(a[field]) : normalizeCycleValue(a[field]);
    const right = field === "start" || field === "end" ? comparableDate(b[field]) : normalizeCycleValue(b[field]);
    return left === right;
  });
}
...
function isAutoGeneratedPaymentRow(row) {
  ...
  return key.includes("|auto-next|") || note.includes("自動帶入");
}
...
function paymentRowsFor(venue, month, year = activeYear) {
  ...
  generatedPaymentRowsFor(venue, month, Number(year)).forEach((generatedRow) => {
    ...
    if (rows.some((row) => samePaymentPeriod(row, generatedRow))) return;
...
function isContractConfirmationRow(row, year = activeYear, monthLabel = activeMonth) {
  ...
  if (String(row?.note || "").includes(contractConfirmationNote)) return true;
  if (!reachesOrPassesContractEnd(row, year, monthLabel)) return false;
  return !hasFutureNextPayment(row, year, monthLabel);
}
...
function serviceKindFor(row) {
  ...
  if (section.includes("辦公室")) return "辦公室";
  if (section.includes("自由座")) return "自由座";
  if (section.includes("營登") || section.includes("年繳") || section.includes("2Y")) return "營登";
  return "待確認";
}
...
function normalizeManualDraftItem(item) {
  ...
  if (!manualDraftLooksLikePayment(normalized)) return normalized;
  const ref = normalizedRefs[0];
  if (!ref) return normalized;
  const row = paymentRowForRef(ref, normalized);
  if (!row || !isContractConfirmationRow(row, ref.year, ref.month)) return normalized;
  ...
  venueKeys.forEach((venue) => {
    monthLabels.forEach((month) => {
      paymentRowsFor(venue, month, year).forEach((row) => {
        ...
        if (!item) return;
        if (manualKeys.has(autoDraftKey(item))) return;
        generated.push(item);
      });
    });
  });
...
function refreshDraftItems() {
  ...
  const normalizedManual = manualDraftItems
    .filter(manualDraftIsLive)
