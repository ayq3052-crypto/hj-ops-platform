...
function sectionFromCrmServiceAndCycle(service, cycle, company) {
...
function saveRowsFor(venue, month, rows, year = activeYear) {
  ...
  const cleanedRows = removeSameMonthRenewalDuplicates(
  ...
  try {
    ...
    if (Array.isArray(saved)) {
      const repaired = ensureRowKeys(
        normalizeSectionGroups(removeSameMonthRenewalDuplicates(repairSavedRows(saved, baseRows, venue, month, year), venue, month, year)),
        venue,
        month,
        year,
      );
      ...
      const rows = ensureRowKeys(
        normalizeSectionGroups(removeSameMonthRenewalDuplicates(merged, venue, month, year)),
        venue,
        month,
        year,
      );
      ...
      const savedRows = ensureRowKeys(
        normalizeSectionGroups(
          removeSameMonthRenewalDuplicates(
            saved.map((row) => normalizeRowSemantics(row, venue)),
            venue,
            month,
            year,
          ),
